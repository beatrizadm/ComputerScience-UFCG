...
public class Jogo {
	...
	public void rodada() throws EstrategiaInvalidaException, JogadaInvalidaException {
		...
		if (jogador1.getNumPecas() == 0) {
			...
			this.pontuacaoVitoria = contaPontuacao(jogador1);
		...
		if (jogador2.getNumPecas() == 0) {
			...
			this.pontuacaoVitoria = contaPontuacao(jogador2);
			this.finalizado = true;
	...
	/**
	 * Metodo que define a pontuacao do jogador
	 * @param jogador: jogador
	 * @return a pontuacao do jogador
	 */
	private int contaPontuacao(Jogador jogador) {
		if (jogador.getUltimaPeca().getNumEsquerdo() == jogador.getUltimaPeca().getNumDireito()) {
			
			// bate nas duas cabecas com um carrocao
			if (jogador.getUltimaPeca().encaixa(this.mesa.getNumNaDireita()) &&
				jogador.getUltimaPeca().encaixa(this.mesa.getNumNaEsquerda())) {
				return 6;
			}
			
			// bate em uma cabeca com um carrocao
			else {
				return 2;
			}
		}
		
		//bate nas duas cabecas com uma peca normal
		else if(jogador.getUltimaPeca().encaixa(this.mesa.getNumNaDireita()) &&
				jogador.getUltimaPeca().encaixa(this.mesa.getNumNaEsquerda())) {
			return 3;
		}
	 ...
	 * @return 
	 * 
	 * @throws EstrategiaInvalidaException Se a estrategia de um dos jogadores
	 *                                     decidir jogar uma peça que ele nao
	 *                                     possui.
	 * @throws JogadaInvalidaException     Se a peca escolhida por algum dos
	 *                                     jogadores nao encaixar na mesa.
	 */
...
//		System.out.println("==> final: " + (venceu == null ? "EMPATE" : venceu + " VENCEU") + "\n");
	 ...
	 */
	public void jogaJogada(Jogador jogador, Jogada jogada) throws JogadaInvalidaException, EstrategiaInvalidaException {
		if (jogada.getTipo() == TipoJogada.PASSA) {
			// checa se tem peca que possa jogar
			if (jogador.temPecaParaJogar(mesa)) {
				throw new EstrategiaInvalidaException("Nao pode passar a vez se tiver pecas disponiveis para serem jogadas");
			}
			return;
		}
		...
		default:
			throw new JogadaInvalidaException("Unexpected value: " + jogada.getTipo());
		}
	...
	/**
	 * @return Id do vencedor, ou null caso o jogo nao esteja finalizado.
	 */
	...
	/**
	 * @return a pontuacao do jogador vitorioso
	 */
	public int getPontuacao() {
		return this.pontuacaoVitoria;
	}
